    ...
    fetch(url, options)
      ...
      .then((data) => {
        ...
        if (data.message === 'jwt expired') {
          ...
          alert(
            'you are not creator of this comment or this post. You dont have permission to delete this comment'
          );
          return false;
        } else {
          setComments(data);
    ...
    fetch(url, options)
      ...
      .then((data) => {
        ...
        if (data.message === 'jwt expired') {
          ...
          alert(
            'you are not creator of this comment you dont permission to do edit'
          );
          return false;
        } else {
          setCommentId(true);
          ...
          {posts.length > 0 &&
            posts.map((post) => (
              <li key={post.id}>
                  ...
                  comments.map((comment) =>
                    comment.postId === post.id ? (
                      <div key={comment.id} className="comments">
                        ...
                        <p>
                          <strong>Created By: </strong> {comment.authorName}{' '}
                          <strong>At:</strong> {comment.date}
                        </p>
                        <button value={comment.id} onClick={handleDelete}>
                          Delete
                        </button>
                        <button value={comment.id} onClick={handleEdit}>
                          Edit
                        </button>
                      </div>
                    ) : (
                ...
                <button value={post.id} onClick={handleComment}>
                  Add-Comment
                </button>
              </li>
            ))}
